...
bool Damage::init(Hero * hero, int type) {
	...
	case(DAMAGE_YANMO):
		// this->initSprite("BigFire.png");
		this->setContentSize(Size(5, 10));
		this->initPhysicsBody();
		this->m_fDamageValue = 0.5;
		this->setPosition(heroPos + ( dir ? Point(heroSize.width/2, -30) : Point(-heroSize.width/2, -30)));
		this->runAction(Sequence::create(MoveBy::create(0.2f, Point(dir ? 15 : -15, 0)), RemoveSelf::create(), nullptr));
		break;
	case(DAMAGE_YEYAN):

...
void Hero::initPhysicsBody() {
	...
	body->setCategoryBitmask(PhysicsMask::hero);
	body->setContactTestBitmask(PhysicsMask::terrain | PhysicsMask::damage);
	body->setCollisionBitmask(PhysicsMask::terrain);
...
void Hero::causeDamage(float damageValue){
	this->m_fHpt -= damageValue;
	CCLOG("causeDamage %d\n",damageValue);
}
